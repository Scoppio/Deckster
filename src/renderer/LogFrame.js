...
export const LogFrame = ({gameState, height, playerRef }) => {
  return (
    <div style={{width: "100%", height: `${height}vh`, background: "green", overflowY: "auto", lineHeight: "20px"}}>
      {gameState.log.length > 0 && (
        <span aria-live="assertive" aria-atomic="true" ref={playerRef.log}>
          <p style={{margin: "0"}}>{gameState.log[0]}</p>
        </span>
      )}
      {gameState.log.slice(1).map((logEntry, idx) => <p key={idx + 1} style={{margin: "0"}}>{logEntry}</p>)}
    </div>
  )
}
