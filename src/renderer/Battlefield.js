...
export const StaticBattlefield = ({ gameState, playerRef, playerNumber, player, landsOnNorth, heightVh }) => { 
  ...
  <div className="row" 
    ...
    aria-describedby={gameState.ariaHelper.cardsOnTheTable(playerNumber)}>
      ...
      {
        player[landsOnNorth ? "land_zone_battlefield" : "front_battlefield"].map((card, index) => (<StaticImgCard card={card} key={index} size={"small"} tabIndex={index + player.tabIndices.front_battlefield} cardHeight={100} />))
      }
      </CardHolder>
      <CardHolder style={{height: `${lineVh}vh`, padding: "2px"}}>
      {
      ...
      <CardHolder style={{height: `${lineVh}vh`, padding: "2px"}}>
      {
        player[landsOnNorth ? "front_battlefield" : "land_zone_battlefield"].map((card, index) => (<StaticImgCard card={card} key={index} size={"small"} tabIndex={index + player.tabIndices.land_zone_battlefield} cardHeight={100} />))
      }
      </CardHolder>
  </div>
)}
  ...
  return (
    <div className="row" 
      ...
      <Droppable droppableId="front_battlefield" direction="horizontal">
        {(provided) => (
          <CardHolder {...provided.droppableProps} ref={provided.innerRef} style={{height: `${lineVh}vh`, padding: "2px"}}>
            {player.front_battlefield.map((card, idx) => <ImgCard key={card._uid} size={"small"} idx={idx} card={card} tabIndex={idx + player.tabIndices.front_battlefield} cardHeight={100} />)}
      ...
      <Droppable droppableId="back_battlefield" direction="horizontal">
        {(provided) => (
          <CardHolder {...provided.droppableProps} ref={provided.innerRef} style={{height: `${lineVh}vh`, padding: "2px"}}>
            {player.back_battlefield.map((card, idx) => <ImgCard key={card._uid} size={"small"} idx={idx} card={card} tabIndex={idx + player.tabIndices.back_battlefield} cardHeight={100} />)}
            {provided.placeholder}
          </CardHolder>
        )}
      </Droppable>
      <Droppable droppableId="land_zone_battlefield" direction="horizontal">
        {(provided) => (
          <CardHolder {...provided.droppableProps} ref={provided.innerRef} style={{height: `${lineVh}vh`, padding: "2px"}}>
            {player.land_zone_battlefield.map((card, idx) => <ImgCard key={card._uid} size={"small"} idx={idx} card={card} tabIndex={idx + player.tabIndices.land_zone_battlefield} cardHeight={100} />)}
            {provided.placeholder}
          </CardHolder>
        )}
      </Droppable>
